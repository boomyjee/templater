...
ui.presetSwitcherCombo = ui.presetCombo.extend({
    ...
    getLabel: function () {
        ...
        {
            if (this.options.repository[this.value.type] 
                && this.options.repository[this.value.type].switcherLabel) 
            {
                ...
                var val = this.options.repository[this.value.type].switcherLabel(this.getValue(),this);
            } else {
                var val = "<span class='button-label'>"+this.value.type+"</span>";
            }
