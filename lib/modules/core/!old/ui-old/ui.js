...
Component.app.bind("component-controls",function(d,e){
    ...
    if (!cmp.controls) cmp.controls = [];
    if (!cmp.overlayControls) cmp.overlayControls = [];
    
    if (cmp.parent && cmp.parent.value.type=="container") {
        cmp.controls.push(
            ui.label({template:"Layout:",margin:"0px 10px 0px"}),
            ui.layoutCombo({name: "layout."+id,margin:"0px 10px 0 10px",width:"100%",inline:true,height:82,switcherWidth:200})
        );
