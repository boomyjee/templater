...
ui.columnResizer = ui.control.extend({
    init: function (o) {
        this._super(o);
        this.element = $("<div class='resizer right-resizer'>");
        ...
        setTimeout(function(){
            me.element.draggable({
                ...
                start: function () {
                    ...
                    me.prevValue = false;
                    me.left = me.options.cmp.componentHandle.offset().left;
                },
                drag: function (e,ui) {
                    var left = me.left;
                    var right = e.clientX;
                    ...
                    if (columns && me.prevValue!=columns) {
                        me.setValue({type:"column",column:columns});
                        me.options.cmp.element.css({width:width+"%"});
                        Component.previewFrame.updateHandles();
                        me.prevValue = columns;

                        setTimeout(function(){
                            ui.helper.height(me.element.height());
                        },1);
                    }
                },
                stop: function () {
                    ...
                    me.trigger("change");
                    me.options.cmp.element.css({width:""});
...
Component.app.bind("component-controls",function(d,e){
    ...
    if (cmp.parent && (cmp.parent.value.type=="container" || cmp.parent.value.type=="form")) {
        if (!cmp.rightResizer) {
            cmp.rightResizer = ui.columnResizer({name:"layout."+id,cmp:cmp});
            cmp.rightResizer.element.appendTo(cmp.controlsRight);
        }
    }
