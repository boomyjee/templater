ui.templaterCodeTab = ui.codeTab.extend({
    ...
    init: function (o) {
        if (!this.Class.optionsButton) {
            this.Class.optionsButton = ui.optionsButton({});
            // hack : remove theme options
            this.Class.optionsButton.dialog.element.children().eq(0).children().slice(-2).remove();
        }
        ...
        this.element.bind("contextmenu",function(e){
            e.preventDefault();
            me.Class.optionsButton.dialog.open();
            return false;
        });
    }
});
