...
ui.backgroundCombo.gradient = ui.composite.extend({
    switcherLabel: function (val) {
        val = val || {};
        var c1 = teacss.functions.color(val.color);
        var c2 = teacss.functions.color(val.color_2);
        return ui.fillCombo.colorLabel(
            "linear-gradient("+c1+","+c2+")",
            ui.imageCombo.iconWidth,
            ui.imageCombo.iconHeight,
            0
        );
    },
    colorLabel: true,
    defaultItem: { value: {color: false,color_2:false} }
},{
    randomize: function () {
        return {
            color: this.items[0].randomize(),
            color_2: this.items[1].randomize()
        }
    }
}).extendOptions({
    label: "Gradient", skipForm: true,
    layout: { margin:"10px 10px 5px", width:150 },
    items: [{
        name:"color", type: "fillCombo", label: "top"
    },{
        name:"color_2", type: "fillCombo", label: "bottom"
    }]
});
