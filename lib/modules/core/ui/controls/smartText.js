...
fabric.TextCanvas = fabric.util.createClass(fabric.Canvas, {
    setValue: function (bg) {
        ...
        if (shadowColor) {
            ...
            text.setShadow(sh);
            text.left = Math.max(sh.blur - sh.offsetX,0);
            text.top = Math.max(sh.blur - sh.offsetY,0);
        } else {
...
var CanvasWrapper = {
    init: function () {
        ...
        this.canvas.setDimensions({width:1200,height:500});
    ...
    getRect: function () {
        ...
        var dw = 0, dh = 0;
        var sh = this.canvas.text.getShadow();
        if (sh) {
            dw = Math.max(sh.blur - sh.offsetX,0) + Math.max(sh.blur + sh.offsetX,0);
            dh = Math.max(sh.blur - sh.offsetY,0) + Math.max(sh.blur + sh.offsetY,0);
        }
        rect.width += dw;
        rect.height += dh;
        return rect;
...
ui.smartText = ui.panel.extend({},{
    ...
    setValue: function (val) {
        ...
        this.strokeColorCombo.setValue(val.strokeValue);
        ...
        this.shadowColorCombo.setValue(val.shadowColorValue);
    },    
