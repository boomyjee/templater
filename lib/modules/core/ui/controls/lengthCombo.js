ui.lengthCombo = ui.combo.extend({
    init: function (options) {
        ...
        this._super($.extend({
            preview: true, 
            min: 0, max: 100, units: 'px', labelUnits: false,
            options: [0,30,50,70,100],
            icons: { secondary: 'ui-icon-triangle-1-s' },
            labelTpl: function () {
                var val = me.value===undefined ? false : me.value;
                var txt = val;
                if (me.options.labelUnits) {
                    txt = txt + me.options.labelUnits;
                }
                $.each(me.options.options,function(){
                    if (this.value==val && this.label)
                        txt = this.label;
                });
                var ret = "<span class='button-label' style='margin-right:4px'>"+txt+"</span>";
                if (label) ret = label + ": "+ret;
                return ret;
            },
            items: function() {
                var items = [];
                ...
                for (var i=0;i<this.options.options.length;i++) {
                    ...
                    if (!isNaN(val)) {
                        ...
                        if (this.options.labelUnits)
                            lbl += this.options.labelUnits;
                        else
                            lbl += this.options.units;
                        items.push({value:val,label:lbl});
                    } else {
                        if (!val.hidden) items.push(val);
                    }
                }
                ...
                text.change(function() { 
                    ...
                    if (this.value===val+me.options.units) return;
    ...
    setValue: function (val) {
        return this._super(val===undefined ? false:val);
    }
});
