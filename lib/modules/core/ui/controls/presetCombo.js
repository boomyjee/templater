ui.presetCombo = ui.combo.extend({
    ...
    refresh: function () {
        ...
        this.itemPanel.sortable({
            ...
            intersection: function (item,widget) {
                var isOverElementHeight = $.ui.isOverAxis(widget.positionAbs.top + widget.offset.click.top, item.top, item.height),
                    isOverElementWidth = $.ui.isOverAxis(widget.positionAbs.left + widget.offset.click.left, item.left, item.width),
    ...
    editPreset: function (item,item_el) {
        ...
        if (me.Class.editor && item==me.Class.editor.currentItem) return;
