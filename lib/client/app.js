    ...
    publish: function () {
        ...
        teacss.build("templater_makefile.tea",{
            callback: function (files) {
                ...
                ui.alert = ui.alert || function(msg,title) {
                    window.alertDialog = window.alertDialog || ui.dialog({
                        modal: true,
                        width: 200,
                        resizable: false,
                        draggable: false,
                    ...
                    window.alertDialog.element.dialog("option","title",title || _t('Document alert'));
                    window.alertDialog.element.html("<p>"+msg+"</p>");
                    window.alertDialog.open();
                }
                
                function publish_request() {
                    me.request("publish",{files:files},function(){
                        ui.alert('Publish success','Publish');
                    });
                }
            }
        });
