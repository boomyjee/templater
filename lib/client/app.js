...
exports = ui.Control.extend({
    ...
    init: function (options) {
        ...
        teacss.jQuery(function(){
            ...
            me.request('load',{cache:1},function (data){
                ...
                var js_list = [];
                $.each(me.options.modules,function(){
                    if (this[0]!='/')
                        js_list.push("../modules/"+this+"/"+this+".js");
                    else
                        js_list.push(this);
                });
    ...
    initUI: function () {
        ...
        me.frame.bind("init",function(){
            me.templateTabs.bind("select",function(e,tab) { me.selectTemplate(tab.template) });
            me.initTabs();
    ...
    initTabs: function () {
        ...
        while (tpl) {
            ...
            var templateStatus = $("<a href='#' class='template-status'>").on("click",{tpl:tpl},function(e){
                e.preventDefault();
                e.stopPropagation();
            ...
            if (tpl) {
                templateStatus.text("Child template for "+tpl);
            } else {
                templateStatus.text("Standalone template");
            }
        }
    },
    ...
    updateComponentUI: function () {
        ...
        var active = teacss.ui.form.activeForm = {items:[]};
